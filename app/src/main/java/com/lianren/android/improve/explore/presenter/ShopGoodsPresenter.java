...
public class ShopGoodsPresenter implements ShopGoodsContract.Presenter {
    private final ShopGoodsContract.View mView;


    public ShopGoodsPresenter(ShopGoodsContract.View mView) {
        this.mView = mView;
    ...
    @Override
    public void getShopGoods(int shop_id, int source) {
        LRApi.shopGood(shop_id, source, new CommonHttpResponseHandler() {
            @Override
            ...
            public void onSuccess(int statusCode, Header[] headers, String responseString) {
                try {
                    Type type = new TypeToken<ResultBean<ShopGoodsBean>>() {
                    }.getType();

                    ResultBean<ShopGoodsBean> resultBean = AppOperator.createGson().fromJson(responseString, type);
                    if (resultBean.isSuccess()) {
                        mView.showShopGood(resultBean.data);
                    } else {
                        mView.showNetworkError(R.string.tip_network_error);
                    }
                } catch (Exception e) {
