package com.lianren.android.improve.notice;
...
public class NoticeServer extends Service {
    ...
    static void arrivedMsgAction(Context context, NoticeCountBean bean) {
    ...
    private final static int ALARM_INTERVAL_SECOND = 10;
    ...
    private void handleAction(String action, Intent intent) {
        if (FLAG_ACTION_REFRESH.equals(action)) {
            ...
            registerNextAlarm();
            // First notify
            sendBroadcastToManager(NoticeCountBean.getInstance(this));
        } else if (FLAG_ACTION_ARRIVED.equals(action)) {
            // Arrived new message
            NoticeCountBean bean = (NoticeCountBean) intent.getSerializableExtra(EXTRA_BEAN);
            if (bean == null)
                return;
            doNewMessage(bean);
        }
    ...
    private void doNetFinish(ResultBean<NoticeCountBean> bean) {
        log("doNetFinish:" + (bean == null ? "null" : bean.toString()));
        if (bean != null && bean.isSuccess()
                && bean.data != null) {
            doNewMessage(bean.data);
        } else {
            // To register alarm
            registerNextAlarm();
        }
    ...
    private void doNewMessage(@NonNull NoticeCountBean bean) {
        log("doNewMessage:" + (bean.toString()));
        NoticeCountBean notice = NoticeCountBean.getInstance(this);
    ...
    private void sendBroadcastToManager(NoticeCountBean bean) {
    ...
    private void sendNotification(NoticeCountBean bean) {
//        if (bean == null || bean.pair==null||bean.pair.count == 0|| bean.system==null||bean.system.count == 0) {
//            clearNotification();
//            return;
//        }
...
//                this)
//                .setTicker(content)
//                .setContentTitle(getString(R.string.you_have_unread_messages, bean.pair.count))
...
//        if (flag == 0 || NoticeCountBean.getInstance(this).getAllCount() <= 0)
//            return;
