...
public final class NoticeManager {
    ...
    private NoticeCountBean mNotice;
...
//    }
     ...
     * @return NoticeBean
     */
    public static NoticeCountBean getNotice() {
        final NoticeCountBean bean = instance().mNotice;
        if (bean == null) {
            return new NoticeCountBean();
        } else {
            return bean;
        }
    }
    ...
    public static void clear(Context context, int type) {
        if (getNotice().system!=null && getNotice().system.count > 0&&
                getNotice().pair!=null && getNotice().pair.count>0)
     ...
     */
    private void onNoticeChanged(NoticeCountBean bean) {
        mNotice = bean;
        //  Notify all
            ...
            if (intent.getAction() != null) {
                if (NoticeServer.FLAG_BROADCAST_REFRESH.equals(intent.getAction())) {
                    ...
                    if (serializable != null) {
                        try {
                            onNoticeChanged((NoticeCountBean) serializable);
                        } catch (Exception e) {
                            e.fillInStackTrace();
                        }
                    }
    ...
    public interface NoticeNotify {
        void onNoticeArrived(NoticeCountBean bean);
    }
}
