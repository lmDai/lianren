package com.lianren.android.widget.invitetime;
...
public class CalendarView extends LinearLayout {
    ...
    public CalendarView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        disableSeletColor = ta.getColor(R.styleable.MyCalendar_dayOutMonthcolor, context.getResources().getColor(R.color.white));
    ...
    public void renderCalendar(String time) {
        ...
        thirdMonth = months.get(1).getCells().get(20).getMonth();//第三个月
    ...
    private class MainRvAdapter extends BaseQuickAdapter<CalendarCell, BaseViewHolder> {
        ...
        protected void convert(BaseViewHolder helper, final CalendarCell item) {
            ...
            if (allAdapters.get(helper.getAdapterPosition()) == null) {
                subRvAdapter = new SubRvAdapter(R.layout.calendar_text_day, item.getCells());
                allAdapters.put(helper.getAdapterPosition(), subRvAdapter);
                ((RecyclerView) helper.getView(R.id.appoint_calendarview_item_rv)).setAdapter(subRvAdapter);
            } else {
                subRvAdapter = allAdapters.get(helper.getAdapterPosition());
                ((RecyclerView) helper.getView(R.id.appoint_calendarview_item_rv)).setAdapter(subRvAdapter);
            }
            //item 点击事件响应
    ...
    private class SubRvAdapter extends BaseQuickAdapter<Date, BaseViewHolder> {
        ...
        protected void convert(BaseViewHolder helper, Date date) {
            ...
            if ((date.getDate() >= dateFromServer.getDate() && date.getMonth() == firstMonth)
                ...
                ((CalendarDayTextView) helper.getView(R.id.calendar_day_tv)).setBackground(ContextCompat.getDrawable(context,
                        R.drawable.bg_calender_enable));
            } else {
                //不可选时间
                ((CalendarDayTextView) helper.getView(R.id.calendar_day_tv)).setTextColor(disableSeletColor);
                ((CalendarDayTextView) helper.getView(R.id.calendar_day_tv)).setBackground(ContextCompat.getDrawable(context,
                        R.drawable.bg_calender_disable));
            }
            ...
            if (sDateTime != null && eDateTime != null && date.getTime() >= sDateTime.getTime()
                    && date.getTime() < eDateTime.getTime()) {
                if (date.getDay() == 6) {
